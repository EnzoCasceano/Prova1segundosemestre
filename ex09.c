...
void meus_primos(int n, int *p1, int *p2) {
    int ehPrimo = 1, achouMaiorPrimoMenorQueN = 0, achouMenorPrimoMaiorQueN = 0;
    int i, j;
    for (i = n - 1; i > 1 && achouMaiorPrimoMenorQueN == 0; i--) {
        for (j = i - 1; j > 1 && ehPrimo; j--) {
            if (i % j == 0) {
                ehPrimo = 0;
            }
        }
        ...
        if (ehPrimo) {
            *p1 = i;
            achouMaiorPrimoMenorQueN = 1;
        }

        ehPrimo = 1;
    }

    for (i = n + 1; achouMenorPrimoMaiorQueN == 0; i++) {
        for (j = i - 1; j > 2 && ehPrimo; j--) {
            if (i % j == 0) {
                ehPrimo = 0;
            }
        }
