...
#define TRUE 1
#define FALSE 0
    int somaDosPrimos = 0, contadorDoVetor, divisor;
    int ehPrimo = TRUE;

    // percorrendo o vetor
    for (contadorDoVetor = 0; contadorDoVetor < n; contadorDoVetor++) {

        // percorrendo o numero PARA VER SE ELE EH PRIMO
        for (divisor = 2; divisor < v[contadorDoVetor] && ehPrimo; divisor++) {
            if (v[contadorDoVetor] % divisor == 0) {
                ehPrimo = FALSE;
            }
        }

        if (ehPrimo == TRUE && v[contadorDoVetor] != 1) {
            somaDosPrimos += v[contadorDoVetor];
        }

        ehPrimo = TRUE;
    }
    return somaDosPrimos;
}
